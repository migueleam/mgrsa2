...
var RolesG = {
    ...
    RefreshRolesTb: function (rdata) {               

        let contenido = "";
        const odivRoles = document.getElementById(RolesG.settings.container);
        let contx = rdata.substring(2, 3);

        if (rdata !== '' && rdata.substring(2, 3) !== '|') {
            const data = CommonG.CrearMatriz(rdata, false);
            const nRows = data.length;
            contenido = 
                `  
                     <table id="${this.settings.id}"  class="table table-hover table-striped w-100 table-no-bordered m-0 w-100 h-100" style="width:100%">
                        ...
                        for (let i = 0; i < nRows; i++) {
                            contenido += `<tr>
                                 <td>${data[i][7]}</td>
                                 <td>${data[i][3]}</td>
                                 <td class='text-center'><button class="btn btn-xs btn-danger deleteRole" data-roleid="${data[i][1]}" onclick="let r = RolesG.DeleteRole(this);"><span class="fa fa-close"></span> Eliminar</button> </td>
                            </tr>`;
                        }
            contenido += `     </tbody>
                          </table>
